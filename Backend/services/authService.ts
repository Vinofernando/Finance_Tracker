...
type RegisterInput = Pick<User, "username" | "email" | "password">;
export const register = async ({
  username,
  email,
  password,
}: RegisterInput): Promise<{ message: string }> => {
  ...
  try {
    ...
    } else if (msg === "All_field_required") {
...
export const deleteUser = async (userId: number, role: string) => {
  if (!userId || role !== "admin")
    throw { status: 400, message: "UserId undefined" };
